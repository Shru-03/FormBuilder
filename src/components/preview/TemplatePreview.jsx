...
const TemplatePreview = ({ formBgColor, formTextColor }) => {
  ...
  const handleSubmit = (e) => {
    ...
    selectedTemplate.sections.forEach((section) => {
      section.fields.forEach((field) => {
        if (!["heading1", "heading2", "heading3"].includes(field.type)) {
  ...
  const renderField = (field) => {
    const baseStyle = `w-full px-4 py-2 text-[${formTextColor}] border border-gray-200 rounded text-sm shadow-sm focus:outline-none focus:ring focus:ring-black`;
    const errorStyle = errors[field.id] ? " focus:ring-red-500" : "";

    switch (field.type) {
      case "heading1":
        return (
          <h1 className="text-[28px]" style={{ color: formTextColor }}>
            {field.label}
          </h1>
        );
      case "heading2":
        return (
          <h2 className="text-[24px]" style={{ color: formTextColor }}>
            {field.label}
          </h2>
        );
      case "heading3":
        return (
          <h3 className="text-[20px]" style={{ color: formTextColor }}>
            {field.label}
          </h3>
        );
      case "Short Answer":
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-300 peer-checked:bg-black transition-all"></div>
  ...
  return (
    <div className="flex flex-col md:flex-row gap-2 md:gap-6 w-full">
      ...
      <form
        ...
        className="flex-1 p-6 md:max-w-2xl max-w-full border border-gray-300 mx-auto my-4 shadow-xl rounded"
      ...
      >
        ...
        {selectedTemplate.sections.map((section) => (
          <div key={section.id} className="mb-6 border-b border-gray-400 pb-6">
            <h4
              className="text-[23px] uppercase font-medium mb-4"
              style={{ color: formTextColor }}
            >
              {section.title}
            </h4>
              ...
              {section.fields.map((field) => (
                <div key={field.id}>
                  {!["heading1", "heading2", "heading3"].includes(
                    ...
                    <label
                      className="flex gap-1 items-start mb-1 text-sm font-medium"
                      style={{ color: formTextColor }}
                    >
                      {field.label}{" "}
                      {field.required && (
